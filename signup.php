...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($confirm_password) || empty($birthday) || empty($gender)) {
            ...
            try {
                ...
                if ($stmt->fetch()) {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        $stmt = $pdo->prepare("INSERT INTO users (username, email, password, birthday, gender, role, is_admin) VALUES (?, ?, ?, ?, ?, 'user', 0)");
                        $stmt->execute([$username, $email, $hashed_password, $birthday, $gender]);
